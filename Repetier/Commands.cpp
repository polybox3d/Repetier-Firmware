...
void process_command(GCode *com,byte bufferedCommand)
{
  ...
  center of arc when h_x2_div_d is positive ->  x <----- | -----> x <- center of arc when h_x2_div_d is negative
    ...
    switch( com->M ) {
      ...
      case 115: {// M115
#if DRIVE_SYSTEM==3
        out.println_P(PSTR("FIRMWARE_NAME:Repetier_" REPETIER_VERSION " FIRMWARE_URL:https://github.com/skapin/Repetier-Firmware/ PROTOCOL_VERSION:1.0 MACHINE_TYPE:Rostock EXTRUDER_COUNT:" XSTR(NUM_EXTRUDER) " REPETIER_PROTOCOL:2 POLYBOX_VERSION:" XSTR(POLYBOX_VERSION) ));
#else
#if DRIVE_SYSTEM==0
        out.println_P(PSTR("FIRMWARE_NAME:Repetier_" REPETIER_VERSION " FIRMWARE_URL:https://github.com/skapin/Repetier-Firmware/ PROTOCOL_VERSION:1.0 MACHINE_TYPE:Mendel EXTRUDER_COUNT:" XSTR(NUM_EXTRUDER) " REPETIER_PROTOCOL:2 POLYBOX_VERSION:" XSTR(POLYBOX_VERSION) ));
#else
        out.println_P(PSTR("FIRMWARE_NAME:Repetier_" REPETIER_VERSION " FIRMWARE_URL:https://github.com/skapin/Repetier-Firmware/ PROTOCOL_VERSION:1.0 MACHINE_TYPE:Core_XY EXTRUDER_COUNT:" XSTR(NUM_EXTRUDER) " REPETIER_PROTOCOL:2 POLYBOX_VERSION:1" XSTR(POLYBOX_VERSION) ));
#endif
#endif
#if EEPROM_MODE!=0
